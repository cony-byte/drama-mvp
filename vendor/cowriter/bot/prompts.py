...
형식·분량은 아래 **[대본 형식]** 지침을 그대로 따른다(**약 100초**, 90~120초). 첫 씬은 훅, 마지막 씬은 엔딩 훅.
...
바로 사용할 수 있는 **기획안 초안**을 아래 구조 그대로 만든다. 확정본이 아니라 초안이다.
...
- **대사**: `*인물명    대사*` — 인물 이름을 앞에 두고 공백 뒤 **실제 대사**. 숏폼답게 짧고 자연스럽게.
  ★대사 줄은 **인물명부터 대사 끝까지 통째로 별표 하나로 감싼다**(`*...*`) — 예) `*도윤    형은 아무것도 몰라.*`
- **내레이션**: `*인물 (Na)    내레이션*` — 배경·감정·시간 압축은 여주 내레이션으로. 이 줄도 **`*...*`로 통째로 감싼다**.
  예) `*도윤 (Na)  10년 전, 아버지는 스스로 목숨을 끊었다. 사람들은 사업 실패라고 했다.*`
- ★지문(행동·묘사)에는 별표를 쓰지 마라 — **인물이 말하는 대사·내레이션 줄에만** `*...*`.
- ★★출력은 **대본 본문만**. 분량·초 표기(`(총 약 100초 분량)`, `약 N초` 등), 구분선(`---`, `===`),
  머리말·맺음말·메타 주석은 **절대 쓰지 마라**(분량은 뒤 단계가 계산하니 대본에 적지 않는다).

## 분량 = 약 100초 (90~120초, 엄수) — ※아래는 지침일 뿐, 이 수치를 대본에 적지 마라
- 씬 **3~6개**, 군더더기 없이. 대사는 꼭 필요한 것만(씬당 대략 2~6줄).
- 소리 내어 읽어 약 100초를 목표로 — 90초 미만으로 짧지 않게, 120초는 넘기지 마라. 장황한 지문·불필요한 씬 금지.
