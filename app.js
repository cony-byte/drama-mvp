...
function renderGateVerdict(m) {
  ...
  if (m.verdict === "over") {
    gateShow(gateBubble(
      `지금 대본은 약 <b>${m.total}초</b> 분량이에요 (권장 ${m.min}~${m.max}초보다 길어요).<br><small>${gateScenes(m)}</small><br>이대로 만들면 내용이 잘릴 수 있어요. 제가 약 ${m.target}초로 <b>압축</b>해드릴까요? 직접 줄이고 싶으시면 아래 버튼을 눌러 안내를 확인해주세요.`,
...
async function gateAutofit(mode) {
  ...
  try {
    ...
    if (m.verdict === "ok") {
      ...
      gateShow(gateBubble(`압축해도 약 <b>${m.total}초</b>라 한 화(${m.min}~${m.max}초)엔 여전히 길어요.<br>두 화로 <b>나눠서</b> 1화만 쓸까요? 직접 더 줄이고 싶으시면 아래 버튼을 눌러주세요.`,
...
$("durationGateChat").addEventListener("click", (e) => {
  ...
  if (act === "revise") {
    // 직접 수정 경로 — 확정하지 않고(잠금 X) 친절히 안내한다. 대본은 이미 편집 가능 상태이므로
    // 편집칸으로 스크롤해 바로 고치게 하고, 다 고치면 [이 대본으로 확정]을 다시 눌러 재측정한다.
    gateShow(gateBubble(
      "네, 좋아요! 위에서 대본을 직접 고친 뒤 다시 <b>[이 대본으로 확정]</b>을 눌러주시면 분량을 다시 재볼게요 🙂",
      [{ act: "close", label: "알겠어요" }]));
